 ...
 *                                                                         *
 ***************************************************************************/
/* This is the generic pic class, for ALL pics supported by pk2cmd
   @author Gibaud Alain */

#ifndef GENERICPK2CMD_HH
#define GENERICPK2CMD_HH
...
class genericPk2cmd : public pic, public ConsoleClient
{
    bool readDevRev (string& dev, string& rev);
    string name2microchipName (const string& n);
    string devicePrefix (string dev);
    map<string, int> fam2ID;
    string device, revision; // for cache
...
public: 
    ...
    int getPk2FamilyID (const string& devName);
};
