    ...
    : HighlighterBase (parent)
{
    ...
    QColor preprocessorColor (0x11, 0x88, 0x11); // green
    QColor gpasmMacroColor (0x00, 0x20, 0xF); // navy blue
    QColor cpikMacroColor (0x00, 0x20, 0xFF); // navy blue
    QColor sfrColor (0xA5, 0x4B, 0xA5); // violine
    QColor instructionColor (0, 0x60, 0xFF); // blue
    QColor stringColor (0xBF, 0x3, 0x3); // bright red
    QColor charConstColor (0xFF, 0x30, 0x0C); // orange
    QColor numberColor (0, 0, 0xFF); // blue
    QColor commentColor (0x60, 0x60, 0x60); // grey
    QColor moduleMarkColor (0xFF, 0x20, 0xFF);
    QColor labelColor(0,0,0); // black

    setLanguage ("picAsm", QStringList ("asm") << "ASM" << "src" << "SRC" << "inc" << "INC"
		 << "slb" << "SLB" << "pic18" << "PIC18");
    setTabLength (5);
    // no indenter for this kind of document
    setIndenter ("");

    // labels begin at column 1
    addRule ("label", "^[a-zA-Z_\\$][a-zA-Z_\\$0-9]*", labelColor, "b");
    ...
    addRules ("directive", directivePatterns, directiveColor, "");
    ...
    addRules ("gpasm_macro", gpasmMacroPatterns, gpasmMacroColor, "b");
    ...
    addRules ("cpik_macro", cpikMacroPatterns, cpikMacroColor, "b");
    ...
    addRules ("instruction", instructionPatterns, instructionColor, "b");
    ...
    addRules ("number", numberPatterns, numberColor);
    ...
    preprocessorPatterns << "\\#define[ ]+[^\\n]*$"
			 ...
			 << "\\b(?:else|endif|endw)\\b" << "\\b[{}]\\b";

    addRules ("preprocessor", preprocessorPatterns, preprocessorColor);

    // single line comment
    addRule ("comment", ";[^\n]*", commentColor);

    // Module marks (in .slb files)
    addRule ("module_mark", "^;<(?:\\+|=|-?)[^>]*>", moduleMarkColor, "bi");
